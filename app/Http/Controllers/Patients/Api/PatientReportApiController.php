...
use Session;
use Lang;
class PatientReportApiController extends Controller {
	...
	public function getindexApi($patient_id = '',$export='')
	{
		...
		{
			$patient_insurances = Insurance::has('patient_insurance')->whereHas('patient_insurance', function($q) use($patient_id){ $q->where('patient_id',$patient_id); })->pluck('insurance_name','id')->all();
	...
	{  
		...
		{	
			if($eligibility_api->api_name == 'eligible' || $eligibility_api->api_name == 'Pverify')
			{ 
				$elibility_arr = [];

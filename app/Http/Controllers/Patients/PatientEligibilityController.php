...
class PatientEligibilityController extends Api\PatientEligibilityApiController {
	...
	{		
		...
		if(Session::get('practice_dbid') != 40)
			return view('patients/eligibility/eligibility', compact('patient_id','eligibility','benefit_verification','patient_insurances'));
		else
			return view('patients/eligibility/eligibility_waystar', compact('patient_id','eligibility','benefit_verification','patient_insurances'));
	}
	...
	public function create($patient_id, $template_id)
	{
		...
		$eligibility = $api_response_data->data->templates;
		//dd($templates);
		...
		return view('patients/eligibility/create', compact('insurance','patient_id', 'eligibility'));
	}
   ...
   public function eligibilitytemplate($patient_id)
	{		
		...
		$benefit_verification = $api_response_data->data->benefit_verification;
		$patient_insurances = $api_response_data->data->patient_insurances;
		$benifit_templates = $api_response_data->data->benifit_templates;
		return view('patients/eligibility/eligibilitytemplate', compact('patient_id','eligibility','benefit_verification','patient_insurances', 'benifit_templates'));
	}
}

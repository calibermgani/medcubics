...
class ModifierLevelApiController extends Controller 
{
	...
	{
		$modifiers = Modifier::with('modifierstype')->where('modifiers_type_id', '2');
		if($export != "")
		{	
	...
	public function getEditApi($id)
	{
		...
		{
			$modifiers 			= 	Modifier::where("id",$id)->where('modifiers_type_id', '2')->first();
			$modifierstype 		= 	Modifierstype::pluck('modifiers_types','id')->all();
	...
	public function getDeleteApi($id)
	{
		...
		if(Modifier::where('id',$id)->where('modifiers_type_id', '2')->count()>0 && is_numeric($id))
		{
			Modifier::where('id',$id)->where('modifiers_type_id', '2')->delete();
	...
	{	
		...
		if(Modifier::where('id', $id )->where('modifiers_type_id', '2')->count()>0 && is_numeric($id))
		{
			$modifiers = Modifier::with('user')->where('modifiers_type_id', '2')->find ($id);

...
class DashboardApiController extends Controller {
    ...
    public function getUnBilledApi() {
        //$resp = Helpers::getClaimStats('unbilled');
        //return $resp['unbilled']['total_amount'];
        return $this->getClaimUnbilledTotalCharge();
    }

    //Unbilled Monthly variations
    public function getUnbilledPercentageApi() {        
        return $this->getCurrentMonthUnbilledPercentage();
    }
    ...
    public function getEdiRejectionApi() {        
        $resp = $this->getClaimStats('rejected');
        return $resp['rejected']['total_amount'];
        //return $this->getEdiRejectionTotal();
    }

    //EDI Rejection Monthly variations
    public function getEdiRejectionPercentage() {        
    ...
    public function getBilledApi() {
        //$resp = $this->getClaimStats('billed');
        // return $resp['billed']['total_amount'];           
        return $this->getClaimBilledTotalCharge();
    }

    public function getBilledPercentageApi() {        
        return $this->getCurrentMonthBilledPercentage();
    }

    //Dashboard Top: Insurance Payment
    //Desc: Insurance payment received for the current month
    //Tech: Sum of insurance_paid
    public function getInsPaymentApi() {
    $getinsPmt = Helpers::getCurrentMonthYearTotalCollections();
    ...
    public function getDashboardChartApi() { 
        ...
        $chargeBar = DB::table('claim_info_v1')->join('pmt_claim_fin_v1', 'claim_info_v1.id', '=', 'pmt_claim_fin_v1.claim_id')->select(DB::raw('sum(claim_info_v1.total_charge) as `total_charge`'), DB::raw("DATE_FORMAT(CONVERT_TZ(claim_info_v1.created_at,'UTC','".$practice_timezone."'),'%M') as monthNum"))
            ...
            ->get('patient_paid', 'monthNum', 'total_charge');             
           // $chargeBar = (array)$chargeBar;  
            
        $i = 0;
        $data = [];
        foreach ($chargeBar as $key => $value) {  
            $data[$value->monthNum]['Charge'] = isset($value->total_charge) ? $value->total_charge : 0;
        }
        
        $patientPmt = $patientRefundPmt = [];
        $patientPmt = PMTInfoV1::where('pmt_method','Patient')->whereIn('pmt_type',['Payment','Credit Balance'])->where('void_check',Null)
        ...
        $patientRefundPmt = PMTInfoV1::where('pmt_method','Patient')->whereIn('pmt_type',['Refund'])
        ...
        $insurancePmt = PMTClaimTXV1::where('pmt_method','Insurance')->whereIn('pmt_type',['Payment','Refund'])
                        ->select(DB::raw('sum(`total_paid`) as `total_paid`'),DB::raw("DATE_FORMAT(CONVERT_TZ(created_at,'UTC','".$practice_timezone."'),'%M') as monthNum"))
                        ->groupBy(DB::raw("month(CONVERT_TZ(created_at,'UTC','".$practice_timezone."'))"))
                        ->where('deleted_at',Null)
                        ->whereRaw('year(created_at) = year(UTC_TIMESTAMP())')
        ...
        $adjPmt = PMTClaimTXV1::whereIn('pmt_method',['Insurance','Patient'])->whereIn('pmt_type',['Payment','Adjustment'])
    ...
    public function getCleanClaimsApi() {
        $total_claims = ClaimTXDESCV1::whereIn('transaction_type', ['Submitted', 'Resubmitted'])->whereRaw('DATE(created_at) = DATE(UTC_TIMESTAMP())')->count();
        
        $accepted_claims = ClaimTXDESCV1::whereIn('transaction_type', ['Clearing House Accepted'])->whereRaw('DATE(created_at) = DATE(UTC_TIMESTAMP())')->count();
        if ($total_claims != '0')
    ...
    public function getInsPercentageApi() {       
        return $this->getCurrentMonthInsPaidPercentage();
    }
   
  
    public function getPatPercentageApi() {        
        return $this->getCurMonPatPercentage();
    } 

    public function getOutstandingArPercentageApi() {        
        return $this->getCurMonOutstandingArPerc();
    }
	// Added where null condition in claims table
	// Revision 1 : 13 Sep 2019 : Selva
    /* Dashboard Top: Ends */
     /** To Do List* */
    public function getReadyToSubmitApi($claimCount = 0) {   // Get Value
        $ReadyToSubmitValue = ClaimInfoV1::whereIn('status', ['Ready'])->whereNull('deleted_at');
    ...
    public function getChargesHoldApi($claimCount = 0) {
        $chargesHold_claim = ClaimInfoV1::whereIn('status', ['Hold'])->whereNull('deleted_at');
    ...
    public function getDeniedApi($claimCount = 0) {
        $chargesDenied_claim = ClaimInfoV1::where('status', "Denied")->whereNull('deleted_at');
    ...
    public function getPendingApi($claimCount = 0) {
        $chargesPending_claim = ClaimInfoV1::where('status', "Pending")->whereNull('deleted_at');
    ...
    public function getNetCollectionsApi() {        
        return $this->getClaimTotalCollections();
    }
   
     public function getERAApi($eraCount = 0) {
    ...
    public function getAgingPiePercentValueApi0_30($total_charge = 0) {
        $last_month_carbon = 30;
        $current_month = 0;
        $maincond = $this->getClaimsTotalCreatedBetween($last_month_carbon, $current_month, $type='Charges');
        return $percentage = ($total_charge != 0) ? round(($maincond / $total_charge) * 100, 2) : 0;
    }

    public function AgingPiePercentValue31_60($total_charge = 0) {
    ...
    public function AgingPiePercentValue61_90($total_charge = 0) {
        //$claims = Claims::with('patient','insurance_details')->where('claim_submit_count','=',0);         
    ...
    public function AgingPiePercentValue91_120($total_charge = 0) {
        $user = Auth::user()->id;
        // $claims = Claims::with('patient','insurance_details')->where('claim_submit_count','=',0);            
    ...
    public function AgingPiePercentValue121_150($total_charge = 0) {
        //$claims = Claims::with('patient','insurance_details')->where('claim_submit_count','=',0);         
    ...
    public function AgingPiePercentValue151_180($total_charge = 0) {
        //$claims = Claims::with('patient','insurance_details')->where('claim_submit_count','=',0);         
        //$last_month_carbon = 180;
        $current_month = 151;
        $maincond = $this->getClaimsTotalCreatedBetween($current_month, 'above', $type='Charges');
        //Claims::where('created_at', '>=', $last_month_carbon)->where('created_at', '<=', $current_month)->sum('total_charge');
        //$total_charge = Claims::sum('total_charge');
        return $percentage = ($total_charge != 0) ? round(($maincond / $total_charge) * 100, 2) : 0;
    }

    public function AgingPiePercentValue180_above($total_charge = 0) {
        //$claims = Claims::with('patient','insurance_details')->where('claim_submit_count','=',0);             
        $current_month = 181;
        $maincond = $this->getClaimsTotalCreatedBetween($current_month, 'above',$type='Charges');
    ...
    public function getLastFirstweekDates() {
    ...
    public function getChargesIndicatorApi() {
        $charges_amount = $this->getClaimsTotalCharges(); // Claims::sum(DB::raw('total_charge'));

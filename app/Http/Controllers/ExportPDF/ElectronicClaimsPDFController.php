...
class ElectronicClaimsPDFController extends Controller
{
    public function index()
    {
    	...
    	try {
			...
			$filename = 'Downloads/ElectronicClaims_'.$created_date.'.pdf';
			$url = Request::url();
	        $request = Request::All();
...
class ElectronicClaimsMypdf extends FPDF
{
    public function header(){
        ...
        $c_width = 295;
        $c_height = 0;
        $text = Practice::getPracticeName().' - Electronic Claims';
        $lengthToSplit = strlen($text);
        ...
        foreach ($this->header as $key => $value) {
            ...
            if ($value == "Check Amt($)" || $value == "AR Bal($)") {
            	$align = "R";
            }else{
            	$align = "L";
            }

...
use App\Http\Controllers\Claims\Api\ClaimApiControllerV1 as ClaimApiControllerV1;
...
class SubmittedClaimsPDFController extends Controller
{
    public function index()
    {
    	...
    	try {
			...
			$filename = 'Downloads/Submitted_Claims_'.$created_date.'.pdf';
			$url = Request::url();
	        $request = Request::All();
			...
			$report_export->report_controller_name = 'ClaimApiControllerV1';
	        ...
	        $controller = New ClaimApiControllerV1;
	        $api_response = $controller->getClaimsDataSearchApi('submitted');
	        ...
	        $pdf = new SubmittedClaims_pdf("L","mm","A4");
			$pdf->SetMargins(2,6);
		...
		foreach($claims as $key => $claim){
            ...
            if($type == 'rejected'){
				...
				if($claim->rendering_provider->provider_name !=''){
					$text = !empty($claim->rendering_provider->short_name)? $claim->rendering_provider->short_name:'-Nill-';
				}else{
					$text = '-Nill-';
				}
				$lengthToSplit = strlen($text);
				...
				if($claim->billing_provider !=''){
					$text = !empty($claim->billing_provider->short_name)? $claim->billing_provider->short_name:'-Nill-';
				}else{
					$text = '-Nill-';
				}
				$lengthToSplit = strlen($text);
				...
				if($claim->facility_detail->facility_name !=''){
					$text = !empty($claim->facility_detail->short_name)? $claim->facility_detail->short_name:'-Nill-';
				}else{
					$text = '-Nill-';
				}
				$lengthToSplit = strlen($text);
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($billed_amount)? Helpers::priceFormat(@$billed_amount,'',1):'-Nill-';
				$lengthToSplit = strlen($text);
				if ($text < 0) {
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($claim->arbal)? Helpers::priceFormat($claim->arbal,'',1):'-Nill-';
				$lengthToSplit = strlen($text);
				if ($text < 0) {
					$pdf->SetTextColor(255, 0, 0);
				}
...
class SubmittedClaims_pdf extends FPDF
{
    public function header(){
        ...
        $c_width = 295;
        $c_height = 0;
        $text = Practice::getPracticeName().' - Submitted Claims';
        $lengthToSplit = strlen($text);
        ...
        $this->header = ['Claim No', 'Dos', 'Patient Name', 'Billed To', 'Category', 'Payer ID', 'Rendering', 'Billing', 'Facility', 'Charge Amt($)', 'AR Bal($)', 'Created Date', 'Submitted Date'];

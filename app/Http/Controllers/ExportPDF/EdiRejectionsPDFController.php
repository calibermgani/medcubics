...
class EdiRejectionsPDFController extends Controller
{
    public function index()
    {
    	...
    	try {
			...
			$filename = 'Downloads/Rejected_claims_'.$created_date.'.pdf';
			$url = Request::url();
	        $request = Request::All();
	        ...
	        $api_response = $controller->getClaimsDataSearchApi('rejected');
	        ...
	        $pdf = new RejectedClaimsPdf("L","mm","A4");
			$pdf->SetMargins(2,6);
	...
	public function BladeContent($claims, $pdf, $type){
		...
		foreach($claims as $key => $claim){
			if(!empty($claim->patient) && !empty($claim->insurance_details)){
				...
				$patient_id = Helpers::getEncodeAndDecodeOfId(@$claim->patient->id,'encode');
                ...
                $billed_amount = Helpers::priceFormat($billed_amount);
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($claim->patient->account_no)? $claim->patient->account_no:'-Nill-';
				$lengthToSplit = 17;
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($claim->insurance_details->short_name)? $claim->insurance_details->short_name:'-Nill-';
				$lengthToSplit = strlen($text);
				$pdf->Vcell($c_width,$c_height,$x_axis,@$text,$lengthToSplit,'', "L");
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($claim->submited_date)? Helpers::dateFormat($claim->submited_date,'claimdate'):'-Nill-';
				$lengthToSplit = strlen($text);
				$pdf->Vcell($c_width,$c_height,$x_axis,@$text,$lengthToSplit,'', "L");

				$x_axis=$pdf->getx();// now get current pdf x axis value
				$text = !empty($claim->rejected_date)? Helpers::dateFormat(@$claim->rejected_date):'-Nill-';
				$lengthToSplit = strlen($text);
				$pdf->Vcell($c_width,$c_height,$x_axis,@$text,$lengthToSplit,'', "L");
			}
   ...
   //      	}
		...
		// $abbreviation = ['abb_billing' => $abb_billing, 'abb_rendering' => $abb_rendering, 'abb_facility' => $abb_facility];
...
class RejectedClaimsPdf extends FPDF
{
    public function header(){
        ...
        $c_width = 295;
        $c_height = 0;
        $text = Practice::getPracticeName().' - Rejected Claims';
        $lengthToSplit = strlen($text);
        ...
        $this->header = ['Claim No', 'Dos', 'Acc No', 'Patient Name', 'Charge Amt($)', 'Payer', 'Payer ID', 'Submitted Date', 'Rejected Date'];

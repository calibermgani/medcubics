...
class PaperClaimsPDFController extends Controller
{
    public function index()
    {
    	...
    	try {
			...
			$filename = 'Downloads/PaperClaims_'.$created_date.'.pdf';
			$url = Request::url();
	        $request = Request::All();
	        ...
	        $api_response = $controller->getClaimsDataSearchApi('paper');
	        ...
	        $pdf = new PaperClaimsMypdf("L","mm","A4");
			$pdf->SetMargins(2,6);
	...
	public function BladeContent($claims, $pdf, $type){
		...
		foreach($claims as $key => $claim){
            ...
            if($type == 'rejected'){
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				if (!empty($claim->insurance_category)) {
					$text = $claim->insurance_category;
				}else{
					$text = '-Nill-';
				}
				$lengthToSplit = strlen($text);
				...
				if($claim->self_pay == 'No'){
					if (!empty($claim->insurance_details->payerid)) {
						$text = @$claim->insurance_details->payerid;
					}else{
						$text = '-Nill-';
					}
				}else{
				...
				if (!empty($claim->created_at)) {
					$text = Helpers::dateFormat(@$claim->created_at, 'date');
				}else{
					$text = '-Nill-';
				}
				$lengthToSplit = strlen($text);
				...
				$x_axis=$pdf->getx();// now get current pdf x axis value
				if (!empty($filled_date)) {
					$text = Helpers::dateFormat(@$filled_date,'date');
				}
				else{
					$text = '-Nill-';
				}
...
class PaperClaimsMypdf extends FPDF
{
    public function header(){
        ...
        $c_width = 295;
        $c_height = 0;
        $text = Practice::getPracticeName().' - Paper Claims';
        $lengthToSplit = strlen($text);
        ...
        $this->header = ['Claim No', 'Dos', 'Patient Name', 'Billed To', 'Category', 'Payer ID', 'Rendering', 'Billing', 'Facility', 'Charge Amt($)', 'AR Bal($)', 'Created Date', 'Filed Date'];

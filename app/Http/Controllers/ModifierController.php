...
class ModifierController extends Api\ModifierApiController 
{
	...
	public function index()
	{
		...
		return view ( 'practice/modifier/modifierlevel1/modifier', compact ( 'modifiers') );
    ...
    public function getModifierExport($export=''){
        ...
        $name = 'Modifier_Level_I_List_' . $date;

        if ($export == 'pdf') {
            $html = view('practice/modifier/modifierlevel1/modifier_export_pdf', compact('modifiers', 'export'));
            return PDF::loadHTML($html, 'A4')->download($name . ".pdf");
        } elseif ($export == 'xlsx') {
            $filePath = 'practice/modifier/modifierlevel1/modifier_export';
	...
	public function create()
	{
		...
		return view('practice/modifier/modifierlevel1/create',  compact('modifierstype','modifiers_type_id'));
	}
	/*** Create page Ends ***/
	...
	public function store(Request $request)
	{
		...
		{
			return Redirect::to('modifierlevel1/create')->withInput()->withErrors($api_response_data->message);
		} 
	}
	/*** Store Function Ends ***/
	...
	public function show($id)
	{
		...
		{
			$modifiers		 	= 	$api_response_data->data->modifiers;
			return view ( 'practice/modifier/modifierlevel1/show', ['modifiers' => $modifiers] );
		}
		else
		{
			return Redirect::to('modifierlevel1')->with('error', $api_response_data->message);
		}
	}
	/*** Show Function Ends ***/
	...
	{		
		...
		if($api_response_data->status == 'success')
		{
			...
			$modifierstype = $api_response_data->data->modifierstype;
			$modifiers_type_id = $api_response_data->data->modifiers_type_id;
			return view('practice/modifier/modifierlevel1/edit', compact('modifiers','modifierstype','modifiers_type_id'));
		}
		else
		{
			return Redirect::to('modifierlevel1')->with('error', $api_response_data->message);
	...
	public function destroy($id)
	{
		...
		if($api_response_data->status == 'success')
		{
			return Redirect::to('modifierlevel1')->with('success',$api_response_data->message);
		}
		else
		{
			return Redirect::to('modifierlevel1')->with('error', $api_response_data->message);
	...
	/*** Modifier import from master starts ***/
	public function importMasterModifiers(){
		return $this->importMasterModifier();
    }
	/*** Modifier import from master ends ***/
}

...
trait ProviderClaimUtil {
    ...
    public function getClaimDetails($claimId = '') {
        $resultSet = [];
        if (!empty($claimId)) {
            $resultSet = ClaimInfoV1::with(['rendering_provider','refering_provider','billing_provider','facility_detail',
    ...
    public function getClaimInsuranceCategory($claimInsuranceId, $payerInsuranceId, $patientId, $insuranceCategory){
        ...
        if(!empty($insuranceId)){
            $InsuranceCategory = PatientInsurance::where('insurance_id',$insuranceId)->where('patient_id',$patientId)->whereIn('category',['Primary','Secondary','Tertiary'])->first();
    ...
    public function getPatientLastPaymentType($patient_id = 0) {
        $pmt_type = PMTInfoV1::where('patient_id', $patient_id)->orderBy('id', 'DESC')->value('pmt_mode');
    ...
    public function getPatientLastStatmentDate($patient_id = 0) {        
        return PatientBudget::where('patient_id',$patient_id)->value('last_statement_sent_date');
    }

    // Patient Budget Amount
    public function getPatientBudgetAmount($patient_id = 0) {
        return Helpers::priceFormat(PatientBudget::where('patient_id',$patient_id)->value('budget_amt'), 'no');
    }

    // Claim wise Listing page                                  
    public function getClaimBilledAmount($claim_id = 0) {
        return Helpers::priceFormat(PMTClaimFINV1::where('claim_id', $claim_id)->value('total_charge'), 'no');
    }

    // Claim Allowed
    public function getClaimAllowedAmount($claim_id = 0) {
        return Helpers::priceFormat(PMTClaimFINV1::where('claim_id', $claim_id)->value('total_allowed'), 'no');
    }
    ...
    public function getClaimFirstSubmissionDate($claim_id = 0) {
        return ClaimInfoV1::where('id', $claim_id)->value('submited_date');
    }

    //  Last claim submission date
    public function getClaimLastSubmissionDate($claim_id = 0) {
        return ClaimInfoV1::where('id', $claim_id)->value('last_submited_date');
    }

    // DOS
    public function getClaimDos($claim_id = 0) {
        return ClaimInfoV1::where('id', $claim_id)->value('date_of_service');
    }
    ...
    public function getProviderClaimUnbilledTotalCharge() {
       $provider_id = Auth::user()->provider_access_id;
       $pro = ClaimInfoV1::join('pmt_claim_fin_v1', 'claim_info_v1.id', '=', 'pmt_claim_fin_v1.claim_id')->select(DB::raw('sum(claim_info_v1.total_charge) - SUM(insurance_paid+patient_paid+withheld+patient_adj+insurance_adj) as unbilled'))->where('claim_info_v1.insurance_id','!=',0)->where('claim_info_v1.rendering_provider_id', $provider_id)->where('claim_info_v1.claim_submit_count', 0)->value('unbilled');
       return $pro;
    }
    ...
    public function getProviderClaimsTotalCharges() {
        $provider_id = Auth::user()->provider_access_id;
        $chargeQry = ClaimInfoV1::where('rendering_provider_id' ,$provider_id)->where(function($qry){
                        $qry->where(function($query){ 
                            $query->where('insurance_id','!=',0)->where('claim_submit_count','>' ,0);
                        })->orWhere('insurance_id',0);
                    });
        $charges_amount = $chargeQry->sum('total_charge'); 
    ...
    public function getProviderClaimsTotalCreatedBetween($start_date, $end_date) { 
        $provider_id = Auth::user()->provider_access_id;
        ...
        if($start_date != 151){
            $balance_amt->whereRaw('claim_info_v1.date_of_service >= DATE(UTC_TIMESTAMP() - INTERVAL '.$start_date.' day)');
    ...
    public function getProviderMonthWiseChargeBar($type = 'total_charge') {
    ...
    public function getInsurancebalance($claim_id = '') {
        ...
        $claim_insurance_due_value = PMTClaimFINV1::where('claim_id', $claim_id)->orderBy('id', 'DESC')->value('insurance_due');
    ...
    public function getProviderClaimStats($type, $date_range='All', $patient_id=0){
        // Billed, Unbilled, 
        $resp = [];
        ...
        */
         $practice_timezone = Helpers::getPracticeTimeZone();   
         $provider_id = Auth::user()->provider_access_id;
         $claims = DB::table('claim_info_v1')->leftjoin('pmt_claim_fin_v1', 'pmt_claim_fin_v1.claim_id', '=', 'claim_info_v1.id')->select(DB::raw('sum(claim_info_v1.total_charge) as `total_amt`'),DB::raw('COUNT(claim_info_v1.id) as `total_charges`'),'claim_info_v1.id')->where('claim_info_v1.rendering_provider_id' ,$provider_id);
        ...
        if(trim($date_range) != '' && trim(strtolower($date_range)) != 'all') {
            ...
            $claims->where(function($query) use ($from, $to, $practice_timezone){ 
                $query->whereRaw('DATE(CONVERT_TZ(claim_info_v1.created_at,"","'.$practice_timezone.'")) >='.$from)->where('DATE(CONVERT_TZ(claim_info_v1.created_at,"","'.$practice_timezone.'")) <='. $to);
            });
        }
        if($patient_id != 0) {
    ...
    public function getProviderClaimBilledTotalCharge() {
        $provider_id = Auth::user()->provider_access_id;
        $practice_timezone = Helpers::getPracticeTimeZone();
        $chargeQry = ClaimInfoV1::whereRaw('MONTH(CONVERT_TZ(created_at,"UTC","'.$practice_timezone.'"))="'.date('m').'"')
 ...
 //whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')
        ->where('rendering_provider_id' ,$provider_id)->where(function($qry){
                        $qry->where(function($query){ 
                            $query->where('insurance_id','!=',0)->where('claim_submit_count','>' ,0);
                        })->orWhere('insurance_id',0);
                    });
//dd($chargeQry->toSql());
    ...
    public function getCurrentMonthInsPaidPercentage() {
        $provider_id = Auth::user()->provider_access_id;
        $last_month = PMTClaimTXV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP() - INTERVAL 1 MONTH) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP() - INTERVAL 1 MONTH)')
        ...
        $current_month = PMTClaimTXV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')
                ->whereHas('claim', function($q)use($provider_id) {
                    $q->where('rendering_provider_id', $provider_id);
                })
                ->where('pmt_method','Insurance')
   ...
   public function getCurMonPatPercentage() {     
        $provider_id = Auth::user()->provider_access_id;
        $currentPatPmt = PMTInfoV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')
             ->whereHas('claim', function($q)use($provider_id) {
                    $q->where('rendering_provider_id', $provider_id);
                })
            ->where('pmt_method','Patient')->whereIn('pmt_type',['Payment','Credit Balance'])->where('void_check',Null)->where('deleted_at',Null)->sum(DB::raw('pmt_amt'));      
        $currentPatRefund = PMTInfoV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')
             ->whereHas('claim', function($q)use($provider_id) {
                    $q->where('rendering_provider_id', $provider_id);
                })
            ->where('pmt_method','Patient')->whereIn('pmt_type',['Refund'])->where('source','posting')->where('void_check',Null)->where('deleted_at',Null)->sum(DB::raw('pmt_amt'));

        $LastPatPmt = PMTInfoV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP() - INTERVAL 1 MONTH) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP() - INTERVAL 1 MONTH)')
            ->whereHas('claim', function($q)use($provider_id) {
                $q->where('rendering_provider_id', $provider_id);
            })
            ->where('pmt_method','Patient')->whereIn('pmt_type',['Payment','Credit Balance'])->where('void_check',Null)->where('deleted_at',Null)->sum(DB::raw('pmt_amt'));       
        $LastPatRefund = PMTInfoV1::with('claim')->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP() - INTERVAL 1 MONTH) = YEAR(UTC_TIMESTAMP() - INTERVAL 1 MONTH)')
            ->whereHas('claim', function($q)use($provider_id) {
                    $q->where('rendering_provider_id', $provider_id);
                })
            ->where('pmt_method','Patient')->whereIn('pmt_type',['Refund'])->where('source','posting')->where('void_check',Null)->where('deleted_at',Null)->sum(DB::raw('pmt_amt'));
    ...
    public function getClaimTotalInsuranceAr() { 
        ...
        $insurance_ar = ClaimInfoV1::join('pmt_claim_fin_v1', 'claim_info_v1.id', '=', 'pmt_claim_fin_v1.claim_id')->where('claim_info_v1.insurance_id', '!=', 0)->where('claim_info_v1.claim_submit_count','!=' ,0)->where('claim_info_v1.rendering_provider_id', $provider_id)
    ...
    public function getClaimTotalPatintAr() {
        ...
        $patient_ar = ClaimInfoV1::join('pmt_claim_fin_v1', 'claim_info_v1.id', '=', 'pmt_claim_fin_v1.claim_id')
                ->where("claim_info_v1.insurance_id", "0")->where('claim_info_v1.rendering_provider_id', $provider_id)
                ->sum(DB::raw('(claim_info_v1.total_charge)-(pmt_claim_fin_v1.patient_paid + pmt_claim_fin_v1.patient_adj+pmt_claim_fin_v1.insurance_paid + pmt_claim_fin_v1.insurance_adj+pmt_claim_fin_v1.withheld)'));
        return Helpers::priceFormat($patient_ar, 'yes');
    }

    public function getClaimTotalOutstandingAr() {
        $provider_id = Auth::user()->provider_access_id;
        $unbilled = ClaimInfoV1::join('pmt_claim_fin_v1', 'claim_info_v1.id', '=', 'pmt_claim_fin_v1.claim_id')->where('claim_info_v1.insurance_id', '!=', 0)->where('claim_info_v1.claim_submit_count', '=', 0)->where('claim_info_v1.rendering_provider_id',$provider_id)->sum(DB::raw('(claim_info_v1.total_charge)-(pmt_claim_fin_v1.patient_paid + pmt_claim_fin_v1.patient_adj+pmt_claim_fin_v1.insurance_paid + pmt_claim_fin_v1.insurance_adj+pmt_claim_fin_v1.withheld)'));
    ...
    public function getProviderCurrentMonthUnbilledPercentage() {  
        $provider_id = Auth::user()->provider_access_id;
        $last_month = ClaimInfoV1::whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP() - INTERVAL 1 MONTH) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP() - INTERVAL 1 MONTH)')->where('rendering_provider_id', $provider_id)->where('insurance_id','!=',0)->where('claim_submit_count', 0)
                        ->sum('total_charge');                
        $current_month = ClaimInfoV1::whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')
                        ->where('rendering_provider_id', $provider_id)->where('insurance_id','!=',0)->where('claim_submit_count', 0)
                        ->sum('total_charge');
    ...
    public function getCurrentMonthEdiRejectionPercentage() {
        ...
        $last_month = ClaimInfoV1::whereIn('status', ['Rejection'])
                      ->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP() - INTERVAL 1 MONTH) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP() - INTERVAL 1 MONTH)')->where('rendering_provider_id',$provider_id)
                      ->sum('total_charge');
    ...
    public function getProviderCurrentMonthBilledPercentage() { 
        $provider_id = Auth::user()->provider_access_id;              
        ...
        $current_month = ClaimInfoV1::where(function($qry){
                        ...
                        })->whereRaw('MONTH(created_at) = MONTH(UTC_TIMESTAMP()) AND YEAR(created_at) = YEAR(UTC_TIMESTAMP())')->where('rendering_provider_id',$provider_id)
                             ->sum('total_charge');                             

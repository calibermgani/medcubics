...
<div class="margin-b-6 margin-t-13"></div>

<table id="claims_table" class="table table-bordered table-striped">    
    <thead>
        <tr>
            <th class="chechbox-readytosubmit">
				<div class="margin-l-3 no-margin" aria-checked="false" aria-disabled="false">
	...
	<tbody>  
		...
		<tr data-url="{{$edit_link}}" class="js-table-click-billing">
			...
			*/ ?> 
			<td>{{ App\Http\Helpers\Helpers::dateFormat($claim->created_at, 'date') }}</td>	
			<td>{{ App\Http\Helpers\Helpers::dateFormat(@$claim->submited_date,'date')}}</td>
			<td>

...
<tr data-url="{{$edit_link}}" class="js-table-click-billing">
	...
	<td>{{ !empty($claim->filed_date)? App\Http\Helpers\Helpers::dateFormat(@$claim->filed_date,'date'):'-Nill-'}}</td>
	<td>
	@if($claim->no_of_issues > 0 && $claim->status == 'Ready')
		...
		@else
		{{ !empty($claim->status)? $claim->status:'-Nill-' }}
		@endif
	</td>
	<td>
		@if(isset($claim->claim_sub_status->sub_status_desc))
			{{ !empty($claim->claim_sub_status->sub_status_desc)? $claim->claim_sub_status->sub_status_desc:'-Nill-' }}
		@else 
			-Nil-
		@endif
	</td>

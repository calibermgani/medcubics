    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
        <div class="box box-view no-shadow ">
            <div class="box-body yes-border border-green border-radius-4">
                {!! Form::open(['onsubmit'=>"event.preventDefault();",'id'=>'js-bootstrap-searchvalidator_edts', 'name'=>'medcubicsform', 'url'=>'reports/financials/workrvureport', 'data-url'=>'reports/financials/workrvureport']) !!}
...
$(document).on('click','.js_search_export_raw', function(){ 
        ...
        var data_arr = [];
            $('select.auto-generate:visible').each(function(){
             ...
             var baseurl = '{{url('/')}}';
            var url = baseurl+"/reports/streamcsv/export/work-RVU-report-js";    
    ...
    today = mm + '/' + dd + '/' + yyyy;
        ...
        ['User : '+user+' | '+today+' '],
        [],
        ['Transaction Date', 'DOS', 'Acc No', 'Patient Name', 'CPT/HCPCS', 'Description', 'Rendering', 'Facility', 'Units', 'Units Charge($)', 'Total Charge($)', 'Work RVU($)']
        ];                        

        var workrvu_list = data.value.workrvu_list;
// console.log(workrvu_list);
        $.each(workrvu_list, function(index, result) {
            // console.log(result);
            // var last_name = result.patient_details.last_name;
            // var first_name = result.patient_details.first_name;
            // var middle_name = result.patient_details.middle_name;
            var patient_name =  result.patient_name;
            var total_amt_charge = (result.units > 0) ? (result.charge / result.units) : result.charge;

            var wa_data_array = [];

            wa_data_array.push(result.transaction_date);
            ...
            wa_data_array.push(result.medium_description);
            wa_data_array.push(result.rendering_provider_name);
            wa_data_array.push((result.facility_name != "" && result.facility_name != null) ? result.facility_name : "");
            wa_data_array.push(result.units);
            wa_data_array.push(total_amt_charge);
            wa_data_array.push(parseInt(result.charge));
            wa_data_array.push(parseInt(result.work_rvu));
        ...
        var merge = [
            { s: {r:0, c:0}, e: {r:0, c:11} },{ s: {r:1, c:0}, e: {r:1, c:11} }, { s: {r:2, c:0}, e: {r:2, c:11} }
            ];
        ...
        saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'Work_RVU_Report.xlsx');

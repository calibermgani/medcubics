...
<div class="row toolbar-header">
    <section class="content-header">
        <h1>
            <small class="toolbar-heading"><i class="livicon med-breadcrum" data-name="barchart"></i> Reports <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> <a href="{{ url('reports/patients/list') }}">Patient Reports</a> <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i>  <span>Statement Status - Detailed</span></small>
        </h1>
        <ol class="breadcrumb">
            
            <li class="dropdown messages-menu hide js_claim_export">
                @include('layouts.practice_module_stream_export', ['url' => 'reports/patientstatementstatus/export/'])
                <input type="hidden" name="report_controller_name" value="ReportController" />
                <input type="hidden" name="report_controller_func" value="patientStatementStatusExport" />
                <input type="hidden" name="report_name" value="Statement Status Detailed" />
            </li>
            ...
            <li><a href="#js-help-modal" data-url="{{url('help/statement_status_report')}}" class="js-help hide" data-toggle="modal"><i class="fa fa-question-circle" data-placement="bottom"  data-toggle="tooltip" data-original-title="Help"></i></a></li>
        </ol>
    </section>
</div>
@stop
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
        <div class="box box-view no-shadow ">
            <div class="box-body yes-border border-green border-radius-4">
                {!! Form::open(['onsubmit'=>"event.preventDefault();",'id'=>'patient_statement_status_report', 'name'=>'medcubicsform', 'url'=>'reports/patientstatementstatus/filter','data-url'=>'reports/patientstatementstatus/filter']) !!}
...
<script>
	...
	var wto = '';
	var url = $('#patient_statement_status_report').attr("action");
	$(document).ready(function(){
		getMoreFieldData();
		$(".selholdBlk").prop("disabled", true);
	});

	// Auto complete set as off to all input fields
	$('input').each(function(){ 

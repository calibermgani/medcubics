    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="dropdown messages-menu hide js_claim_export">
                @include('layouts.practice_module_stream_export', ['url' => '/reports/filter_resultProvider/export/'])
                <input type="hidden" name="report_controller_name" value="ProviderlistController" /> 
                <input type="hidden" name="report_controller_func" value="providerListExport" />
                <input type="hidden" name="report_name" value="Provider Summary" />
            </li>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
        <div class="box box-view no-shadow ">
            <div class="box-body yes-border border-green border-radius-4">
                {!! Form::open(['onsubmit'=>"event.preventDefault();",'id'=>'js-bootstrap-searchvalidator_edt', 'name'=>'medcubicsform', 'url'=>'reports/practicesettings/filter_resultProvider','data-url'=>'reports/practicesettings/filter_resultProvider']) !!}
                ...
                <div id="js_search_date_adj" class="js_date_validation js_date_option js_enter_date no-padding">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-horizontal no-padding js_search_part">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            {!! Form::hidden('practiceoption', 'provider_summary')  !!}
...
<script type="text/javascript">
    ...
    var allcolumns = [];
    var listing_page_ajax_url = api_site_url+"/reports/practicesettings/filter_resultProvider"; 

	//--------------------------------- FORM SUBMIT ----------------------
	$(document).ready(function(){   
		getMoreFieldData();   
		$('[id^=rendering_id]').hide();
		<?php 
		if(!empty($searchUserData))
			foreach(json_decode($searchUserData->search_fields_data,true) as $res){
				if($res['label_name']=='provider_type'){
					if($res['value']==1){?>
						$('[id^=billing_id]').hide();
						$("[id^=rendering_id]").show();
						<?php
					}
					else{?>
						$('[id^=billing_id]').show();
	...
	$("#provider_type.js_select_basis_change").on("click",function(){
		$('[id^=rendering_id]').hide();
		$('[id^=billing_id]').hide();
		if($(this).val() == '1'){
			$("[id^=rendering_id]").show();
			$("[id^=rendering_id] .select2-container").removeClass('hide');
		}
		else{
			$('[id^=billing_id]').show();
			$("[id^=billing_id] .select2-container").removeClass('hide');
		}
	});

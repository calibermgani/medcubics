...
<div class="row toolbar-header">
    <section class="content-header">
        <h1>
            <small class="toolbar-heading"><i class="fa {{Config::get('cssconfigs.admin.ticket')}} font14"></i> Manage Ticket <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> <span>My Ticket</span></small>
        </h1>
        <ol class="breadcrumb">
				...
				<table id="list_noorder" class="table table-bordered table-striped">
					...
					<tbody>
						...
						<tr data-url="{{ url('admin/manageticket/'.$manageticket->id) }}" class="js-table-click clsCursor">
							...
							<td>{{ ($manageticket->status == 'Open' && $manageticket->read == '0')? 'New' : $manageticket->status }}</td>
							<td>{{ $manageticket->notification_sent }}</td>
							<td class="changeassigntype{{$manageticket->ticket_id}}">
								@if($manageticket->status != 'Closed')
									<a data-url="{{ url('admin/assignticket/')}}" data-ticketid="{{$manageticket->ticket_id}}" data-backdrop="false" data-toggle="modal" data-userid="{{ $assigneduserid }}" class="js_ticketassign tooltips"  data-target="#ticketassign_modal" href="#">Reassign</a>
								@endif
							</td>
						</tr>
						@endforeach

<?php 
	$patient_id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId(@$claim_detail_val->patient->id,'encode');
	$claim_id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId(@$claim_detail_val->id,'encode');
	$statelicense = isset($claim_detail_val->refering_provider_id)?App\Models\Provider::checkstatelicense($claim_detail_val->refering_provider_id):'';
	$statelicense_billing = isset($claim_detail_val->billing_provider_id)?App\Models\Provider::checkstatelicense($claim_detail_val->billing_provider_id):'';
?>  
<div id="claim-charge-modal_{{$claim_detail_val->claim_number}}" class="modal fade in">
	{!! Form::open(['onsubmit'=>"event.preventDefault();",'method'=>'POST','name'=>'claim_editcharge_form','id' => 'bootstrap-validator-editcharge_'.@$claim_detail_val->claim_number,'class'=>'popupmedcubicsform js_billingform']) !!}
	<div class="modal-md-650">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title"> Edit Charge - {{$claim_detail_val->claim_number}}</h4>
			</div>
			
			{!!Form::hidden('claim_id',@$claim_id, ['id' => 'js-claim-id'])!!}
			{!!Form::hidden('providertypeid',@$claim_detail_val->refering_provider->provider_types_id,['id' => 'providertypeid'])!!}
			...
			{!!Form::hidden('taxanomy',@$claim_detail_val->billing_provider->taxanomy->code, ['id' => 'providertaxanomy'])!!}
			{!!Form::hidden('upinno',@$claim_detail_val->refering_provider->upin, ['id' => 'upin_no'])!!}
			@if(!empty($claim_detail_val->claim_ids))
			<?php
				$claims_count = explode(',', $claim_detail_val->claim_ids);
				if (count($claims_count) > 1) { 
					$key = array_search($claim_detail_val->id, $claims_count);
			...
			if(is_null($patient_id)){
				$patient_id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId(@$claim_detail_val->patient_id,'encode');;
			} 
			?>
			...
			<div class="modal-body no-padding" >
				<div class="box box-view no-shadow no-border"><!--  Box Starts -->
					<div class="box-body form-horizontal">
									...
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 tab-r-b-1 border-green"><!--  1st Content Starts -->
										...
										<div class="box-body form-horizontal"><!-- Box Body Starts -->
											<div class="form-group-billing">
												...
												@if(empty(@$claim_detail_val))
												{!!Form::hidden('charge_add_type','billing')!!}
												@endif
												<label for="Rendering Provider" class="col-lg-5 col-md-5 col-sm-4 control-label-billing med-green" id="demo">Rend Prov</label> 
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10 select2-white-popup @if($errors->first('rendering_provider_id')) error @endif">
													{!! Form::select('rendering_provider_id', array('' => '-- Select --') + (array)$rendering_providers,  @$claim_detail_val->rendering_provider_id,['class'=>'select2 form-control', 'id' => 'providerpop', 'onChange' => 'getselecteddetail(this.id,this.value, \'Provider\');']) !!}  
											...
											<div class="form-group-billing">
												...
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10 select2-white-popup ">
												   {!! Form::select('refering_provider_id', array('' => '-- Select --') + (array)$referring_providers,  @$claim_detail_val->refering_provider->id,['class'=>'select2 form-control']) !!}
												</div>                                
											</div>

											<div class="form-group-billing">
												<label for="Billing Provider" class="col-lg-5 col-md-5 col-sm-4 control-label-billing med-green">Bill Prov</label> 
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10 select2-white-popup ">
													{!! Form::select('billing_provider_id', array('' => '-- Select --') + (array)$billing_providers,  @$claim_detail_val->billing_provider_id,['class'=>'select2 form-control','id' => 'billingprovider-pop','onChange' => 'getselecteddetail(this.id,this.value, \'Provider\');']) !!}  
											...
											<div class="form-group-billing">
												...
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10 select2-white-popup ">  
													{!! Form::select('facility_id', array(''=>'-- Select --')+(array)$facilities,  @$claim_detail_val->facility_id,['class'=>'select2 form-control','id'=>'facility_id', 'onChange' => 'changeselectval(this.value,\'Facility\', \'\');']) !!}   
											...
											<?php
												...
												if(isset($claim_detail_val->patient->is_self_pay) && ($claim_detail_val->patient->is_self_pay == 'Yes') && !empty($insurance_arr)){
													unset($primary_val);
											...
											<div class="form-group-billing">
												...
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10 select2-white-popup">  
													@if($claim_detail_val->patient->is_self_pay == 'Yes' || empty($insurance_arr))
														{!! Form::select('insurance_id', ['self' => 'Self'], 'self',['readonly' => 'readonly','class'=>'select2 form-control']) !!}
													@else
														{!! Form::select('insurance_id', array('self' => 'Self')+$insurance_arr,!empty($claim_detail_val->insurance_id)?$claim_detail_val->insurance_id:array_keys($primary_val),['readonly' => 'readonly','class'=>'select2 form-control']) !!}
											...
											<div class="form-group-billing"> 
												...
												<div class="col-lg-7 col-md-7 col-sm-7 col-xs-10">
													{!! Form::text('auth_no',@$claim_detail_val->auth_no,['maxlength'=>'25','id'=>'authorization','class'=>'form-control input-sm-header-billing','readonly'=>'readonly']) !!}
										...
										<div class="box-body form-horizontal js-address-class" id="js-address-primary-address">                         
											...
											<div class="form-group-billing">                             
												...
												<div class="col-lg-5 col-md-5 col-sm-4 col-xs-5 ">
													<i class="fa fa-calendar-o form-icon-billing" onclick = "iconclick('admit_date')"></i>  
													{!! Form::text('admit_date',(isset($claim_detail_val->admit_date) && $claim_detail_val->admit_date != '1970-01-01' && $claim_detail_val->admit_date != '0000-00-00')?@date('m/d/Y',strtotime($claim_detail_val->admit_date)):'',['class'=>'form-control call-datepicker dm-date input-sm-header-billing dm-date p-r-0','placeholder'=>"From"]) !!}
												</div>        
													{!!Form::hidden("small_date",null,['id' => 'small_date'])!!}													
												<div class="col-lg-4 col-md-4 col-sm-4 col-xs-5 ">
													{!! Form::text('discharge_date',(isset($claim_detail_val->discharge_date) && $claim_detail_val->discharge_date != '1970-01-01' && $claim_detail_val->discharge_date != '0000-00-00')?@date('m/d/Y',strtotime($claim_detail_val->discharge_date)):'',['class'=>'form-control dm-date input-sm-header-billing call-datepicker dm-date p-r-0','placeholder'=>"To"]) !!}
												</div>
											</div>
											...
											<div class="form-group-billing">
												...
												<div class="col-lg-9 col-md-9 col-sm-8 col-xs-10">
													<i class="fa fa-calendar-o form-icon-billing" onclick = "iconclick('doi')"></i> 
													{!! Form::text('doi',(@$claim_detail_val->doi && $claim_detail_val->doi !='0000-00-00 00:00:00')?date('m/d/Y', strtotime($claim_detail_val->doi)):'',['class'=>'form-control dm-date input-sm-header-billing', 'id' => 'date_of_injury']) !!}
												</div>                          
											</div>   
											...
											<div class="form-group-billing">
												...
												<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 billing-select2-disabled-yellow">
													{!! Form::text('bill_cycle',@$claim_detail_val->patient->bill_cycle,['maxlength'=>'25','class'=>'form-control input-sm-header-billing','readonly'=>'readonly']) !!}
												</div>
												<label for="pos" class="col-lg-1 col-md-1 col-sm-1 col-xs-2 control-label-billing med-green p-l-0">POS</label> 
												<div class="col-lg-4 col-md-4 col-sm-3 col-xs-4">    
													{!! Form::select('pos_id', (array)$pos, @$claim_detail_val->pos_id, ['class'=>'select2 form-control input-sm-header-billing', 'id' => 'pos_id']) !!}
												</div>    
											</div>
											...
											<div class="form-group-billing">
												...
												<?php 
													$copay_detail = (!empty($claim_detail_val->id))?App\Models\Payments\PMTInfoV1::getPaymentInfo($claim_detail_val->id):[];
													$disabled_class = '';
													$readonly_class = '';
												...
												<div class="col-lg-4 col-md-4 col-sm-3 col-xs-5">
													{!! Form::text('copay_amt',(@$claim_detail_val->copay_amt != 0? @$claim_detail_val->copay_amt:''),['class'=>'form-control input-sm-header-billing', 'maxlength' => '7',$readonly_class]) !!}
												</div>    
											</div>
											
											<div class="form-group-billing js-show-check <?php echo $other_type;?>">
												{!! Form::label('mode', 'Check No',  ['class'=>'col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label-billing med-green font600']) !!} 
											...
											<div class="form-group-billing <?php echo $credit;?> js-show-card-type">
												{!! Form::label('mode', 'Card Type',  ['class'=>'col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label-billing med-green font600']) !!} 
												<div class="col-lg-8 col-md-8 col-sm-7 col-xs-10">                                  
											...
											<div class="form-group-billing">
												{!! Form::label('mode', 'Date',  ['class'=>'col-lg-3 col-md-4 col-sm-4 col-xs-12 control-label-billing med-green font600']) !!} 
												<div class="col-lg-8 col-md-8 col-sm-7 col-xs-10">
						...
						@if(!empty($claim_detail_val)) 
						<?php $icd_lists = array_flip(array_combine(range(1, count(explode(',', $claim_detail_val->icd_codes))), explode(',', $claim_detail_val->icd_codes))); ?>              
						<?php $icd = App\Models\Icd::getIcdValues($claim_detail_val->icd_codes); ?>
						@endif
						<div id="js-count-icd">
							<!-- Display ICD orders from E-superbill -->          
						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding margin-t-8 mobile-scroll">                            
							<ul class="billing line-height-26 border-radius-4 no-padding mobile-width billing-charge-table" id="">
									...
									<table class="table-billing-view">
										<thead>
											<tr>
												...
												<th class="td-c-23">ICD Pointers</th>
								...
								<?php
									...
									$count_cnt = 0;
									if (!empty($claim_detail_val)) {
										$cpt_codes = explode(',', $claim_detail_val->cpt_codes);
										$count_cnt = count($cpt_codes);
										if ($count_cnt > 6)
											$count = 10;
										$cpt_icd = explode('::', $claim_detail_val->cpt_codes_icd);
									}
								?>
								
							<!-- Display CPT from E-superbill -->							

							@if(!empty($claim_detail_val->dosdetails))
							<?php if (count($claim_detail_val->dosdetails) > $count) $count = count($claim_detail_val->dosdetails); ?>
							<div class="js-append-parent">
							@for($i=0;$i<$count;$i++)
							<?php 
								$date_to = '';
								$date_from = ''; 
								if (!empty($claim_detail_val->dosdetails[$i]->dos_to)) {
									$date_to = (@$claim_detail_val->dosdetails[$i]->dos_to && $claim_detail_val->dosdetails[$i]->dos_to != '0000-00-00 00:00:00') ? date('m/d/Y', strtotime($claim_detail_val->dosdetails[$i]->dos_to)) : '';
								}
								if (!empty($claim_detail_val->dosdetails[$i]->dos_from)) {
									$date_from = (@$claim_detail_val->dosdetails[$i]->dos_from && $claim_detail_val->dosdetails[$i]->dos_from != '0000-00-00 00:00:00') ? date('m/d/Y', strtotime($claim_detail_val->dosdetails[$i]->dos_from)) : '';
								}							
								$icd_map = isset($claim_detail_val->dosdetails[$i]->cpt_icd_map_key) ? array_combine(range(1, count(explode(',', $claim_detail_val->dosdetails[$i]->cpt_icd_map_key))), explode(',', $claim_detail_val->dosdetails[$i]->cpt_icd_map_key)) : '';
								$style = '';
								if ($i >= 6) {
							...
							<li id = "js-modifier-list-{{$i}}" class="billing-grid js-disable-div-{{$i}}" <?php echo $style; ?>>
								<table class="table-billing-view superbill-claim">
									<tbody>
										<tr>
											<td class="td-c-2"><input tabindex = -1 type="checkbox" id="<?php echo $i; ?>"class="js-icd-highlight flat-red" onClick = "highlightcheckbox(<?php echo $i; ?>, this)"></td>  
											...
											<td class="td-c-8">
												<input type="text" id="<?php echo $i; ?>" readonly="readonly" class="js-cpt billing-noborder" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_code}}" name= <?php echo "cpt[" . $i . "]"; ?> >
												<input type="hidden" class="billing-noborder cpt_amt_<?php echo $i; ?>" value = "{{@$claim_detail_val->dosdetails[$i]->charge}}" name="<?php echo "cpt_amt[" . $i . "]"; ?>">
											</td>
											
											<td class="td-c-4">
											<input type="hidden" class="billing-noborder js-hidden-cpt" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_code}}">
											{!! Form::text('modifier1['.$i.']',@$claim_detail_val->dosdetails[$i]->modifier1,['class'=>'billing-noborder js-modifier bg-white', 'id' =>'modifier1-'.$i ]) !!}</td>
											<td class="td-c-4" >{!! Form::text('modifier2['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier2,['class'=>'billing-noborder js-modifier bg-white', 'id' =>'modifier2-'.$i]) !!}</td>
											<td class="td-c-4">{!! Form::text('modifier3['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier3,['class'=>'billing-noborder js-modifier bg-white', 'id' =>'modifier3-'.$i]) !!}</td>
											<td class="td-c-4">{!! Form::text('modifier4['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier4,['class'=>'billing-noborder js-modifier bg-white', 'id' =>'modifier4-'.$i]) !!}</td>
											...
											<td class="td-c-3"><input class="cpt_unit billing-noborder" type="text" id="<?php echo $i ?>"  maxlength = 5 name=<?php echo "unit[" . $i . "]"; ?> value = "{{@$claim_detail_val->dosdetails[$i]->unit}}" ></td>
											<td class="td-c-6"><input type="text" class = "js-charge form-control input-sm-header-billing billing-noborder" id= "charge_<?php echo $i ?>" name=<?php echo "charge[" . $i . "]"; ?> value = "{{@$claim_detail_val->dosdetails[$i]->charge}}">
											<input type="hidden" class="cpt_allowed_amt_<?php echo $i; ?>" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_allowed_amt}}" name="<?php echo "cpt_allowed[" . $i . "]"; ?>">
											<input type="hidden" class="cpt_icd_map billing-nb" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_icd_code}}" name=<?php echo "cpt_icd_map[" . $i . "]"; ?>  onChange="modelvalue()">
											<input type="hidden" class="cpt_icd_map_key billing-nb" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_icd_map_key}}" name=<?php echo "cpt_icd_map_key[" . $i . "]"; ?> ></td>                              
										</tr>
									</tbody>
								</table>                                     
							</li>                
						...
						@else
						<div class="js-append-parent">                    
							<?php $dos_date = (!empty($claim_detail_val)) ? date('m/d/Y', strtotime($claim_detail_val->date_of_service)) : ''; ?>
							@for($i=0;$i<$count;$i++)
							<?php
								...
								<table class="table-billing-view superbill-claim">
									<tbody>
										<tr>
											...
											<td class="td-c-4">{!! Form::text('modifier1['.$i.']',@$claim_detail_val->dosdetails[$i]->modifier1,['class'=>'billing-noborder js-modifier bg-white', 'maxlength' => 2, 'id' =>'modifier1-'.$i]) !!}</td>
											<td class="td-c-4">{!! Form::text('modifier2['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier2,['class'=>'billing-noborder js-modifier bg-white', 'maxlength' => 2, 'id' =>'modifier2-'.$i]) !!}</td>
											<td class="td-c-4">{!! Form::text('modifier3['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier3,['class'=>'billing-noborder js-modifier bg-white', 'maxlength' => 2, 'id' =>'modifier3-'.$i ]) !!}</td>
											<td class="td-c-4">{!! Form::text('modifier4['.$i.']' ,@$claim_detail_val->dosdetails[$i]->modifier4,['class'=>'billing-noborder js-modifier bg-white', 'maxlength' => 2, 'id' =>'modifier4-'.$i]) !!}</td>
									...
									<input type="hidden" class="cpt_allowed_amt_<?php echo $i; ?>" value = "{{@$claim_detail_val->dosdetails[$i]->cpt_allowed}}" name="<?php echo "cpt_allowed[" . $i . "]"; ?>">
						...
						@endif
							...
							<div class="margin-t-m-8 margin-b-5">
								<span class="append cur-pointer font600 med-green" style="display:none;"><i class="fa fa-plus"></i> Add</span>                
							</div>
						</div>
						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding margin-t-m-13">
							<textarea class="form-control minheight-50" placeholder="Notes" name="note" cols="50" rows="10"></textarea>
						</div>                        
						...
						<div class="box-footer space20">
							<div id="claim_editcharge_footer_{{@$claim_detail_val->claim_number}}" class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								{!! Form::submit('Save', ['class'=>'btn btn-medcubics-small padding-2-8 js-claimeditcharge-form-submit','id'=>'editcharge-form-submit_'.@$claim_detail_val->claim_number]) !!}

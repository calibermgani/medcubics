...
<div class="row toolbar-header">
    <?php $holdoption->id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId($holdoption->id,'encode'); ?>
    <section class="content-header">
        <h1>
            <small class="toolbar-heading"><i class="fa fa-medkit font14"></i> {{$heading}} <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> Account Preference <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> Hold Reason <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> <span>View</span></small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ url('holdoption')}}"><i class="fa {{Config::get('cssconfigs.common.back')}}" data-placement="bottom"  data-toggle="tooltip" data-original-title="Back"></i></a></li>
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin-t-10 margin-b-10">
    @if($checkpermission->check_url_permission('holdoption/{holdoption}/edit') == 1)
    <a href="{{ url('holdoption/'.$holdoption->id.'/edit')}}" class=" pull-right font14 font600 margin-r-5"><i class="fa {{Config::get('cssconfigs.common.edit')}}"></i> Edit</a>
    @endif	
</div>
    ...
    <div class="box box-info no-shadow"><!-- General Info Box Starts -->
        ...
        <div class="box-body form-horizontal margin-l-10">
            <div class="form-group">
                {!! Form::label('Option', 'Hold Reason', ['class'=>'col-lg-2 col-md-2 col-sm-4 col-xs-12 control-label']) !!}  
                <div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
                    <p class="show-border no-bottom">{{ $holdoption->option }}</p>								
                </div>                
            </div>  

			<div class="form-group">
                {!! Form::label('hold_reason', 'Hold Description', ['class'=>'col-lg-2 col-md-2 col-sm-4 col-xs-12 control-label ']) !!}  
                <div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
                    <?php // text box change to textarea the reason is open pop-up but, At the same time form is submited so changed
                    ?>
                    <p class="show-border no-bottom">{{ $holdoption->hold_reason }}</p>	
            ...
            <div class="form-group">
                ...
                <div class="col-lg-3 col-md-4 col-sm-8 col-xs-12">
                    <p class="show-border no-bottom">@if($holdoption->created_by != ''){{ App\Http\Helpers\Helpers::shortname($holdoption->created_by) }} @endif</p>						
                </div>                
            </div> 
            ...
            <div class="form-group">
                ...
                <div class="col-lg-2 col-md-3 col-sm-8 col-xs-12">
                    <p class="show-border no-bottom">@if($holdoption->created_at !='' && $holdoption->created_at !='-0001-11-30 00:00:00' && $holdoption->created_at !='0000-00-00 00:00:00')
                        {{ App\Http\Helpers\Helpers::dateFormat($holdoption->created_at, 'date') }}
                        @endif</p>						
                </div>                
            </div> 
            
            @if(@$holdoption->updated_by != '')
            <div class="form-group">
                {!! Form::label('Updated By', 'Updated By', ['class'=>'col-lg-2 col-md-2 col-sm-4 col-xs-12 control-label']) !!}   						
                <div class="col-lg-3 col-md-4 col-sm-8 col-xs-12">
                    <p class="show-border no-bottom">{{ App\Http\Helpers\Helpers::shortname($holdoption->updated_by) }}</p>
                </div>                
            </div>
            @endif
            
            @if($holdoption->updated_at !='' && $holdoption->updated_at !='-0001-11-30 00:00:00' && $holdoption->updated_at !='0000-00-00 00:00:00')
            <div class="form-group">
                {!! Form::label('Updated On', 'Updated On', ['class'=>'col-lg-2 col-md-2 col-sm-4 col-xs-12 control-label']) !!}   						
                <div class="col-lg-2 col-md-3 col-sm-8 col-xs-12">
                    <p class="show-border no-bottom">
                        {{ App\Http\Helpers\Helpers::dateFormat($holdoption->updated_at, 'date') }}
                    </p>
                </div>                
            </div>
            @endif

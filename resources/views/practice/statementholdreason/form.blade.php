    ...
    <div class="box box-info no-shadow"><!-- General Info Box Starts -->
        ...
        <div class="box-body form-horizontal margin-l-10">
            <div class="form-group">
                {!! Form::label('hold_reason', 'Hold Reason', ['class'=>'col-lg-3 col-md-3 col-sm-4 col-xs-12 control-label star']) !!}  
                <div class="col-lg-4 col-md-6 col-sm-8 col-xs-12 @if($errors->first('hold_reason')) error @endif">
                    ...
                    ?>
                    {!! Form::text('hold_reason',null,['class'=>'form-control','maxlength'=>'100','autocomplete'=>'off']) !!}
			...
			@if(strpos($currnet_page, 'edit') !== false)
				...
				@endif
				*/?>
				<a href="javascript:void(0)" data-url="{{ url('statementholdreason/'.$statementholdreason->id)}}"> {!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
			@else
				<a href="javascript:void(0)" data-url="{{ url('statementholdreason') }}"> {!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
			@endif
		</div>	   
        </div><!-- /.box-body -->

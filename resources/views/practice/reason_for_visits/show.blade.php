...
<?php $reason->id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId($reason->id,'encode'); ?>
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="{{ url('reason')}}"><i class="fa {{Config::get('cssconfigs.common.back')}}" data-placement="bottom"  data-toggle="tooltip" data-original-title="Back"></i></a></li>			
...
@section('practice')
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin-t-10 margin-b-10">   
		@if($checkpermission->check_url_permission('reason/{id}/edit') == 1)
			<a href="{{ url('reason/'.$reason->id.'/edit')}}" class=" pull-right font14 font600 margin-r-5"><i class="fa {{Config::get('cssconfigs.common.edit')}}"></i> Edit</a>
		@endif	
	</div>
        ...
        <div class="box box-info no-shadow"><!-- General Info Box Starts -->
            ...
            <div class="box-body form-horizontal margin-l-10">
                <div class="form-group">
                    {!! Form::label('Reason', 'Reason', ['class'=>'col-lg-2 col-md-2 col-sm-4 col-xs-12 control-label']) !!}   						
                    <div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
                       <p class="show-border no-bottom">{{ $reason->reason }}</p>						
                    </div>                
                </div>   
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
                       <p class="show-border no-bottom">{{ App\Http\Helpers\Helpers::shortname($reason->created_by) }}</p>	
                    </div>                
                </div> 
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
                        <p class="show-border no-bottom">
                            @if($reason->created_at !='' && $reason->created_at !='-0001-11-30 00:00:00' && $reason->created_at !='0000-00-00 00:00:00')
                            {{ App\Http\Helpers\Helpers::dateFormat($reason->created_at, 'date') }}
                            @endif
                        </p>						
                    </div>                
                </div> 
                ...
                @if(@$reason->updated_by != '')
    				<div class="form-group">
    					...
    					<div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
    						<p class="show-border no-bottom">{{ App\Http\Helpers\Helpers::shortname($reason->updated_by) }}</p>	
    					</div>                
    				</div>
    				...
    				<div class="form-group">
    					...
    					<div class="col-lg-4 col-md-5 col-sm-8 col-xs-12">
    						<p class="show-border no-bottom">{{ App\Http\Helpers\Helpers::dateFormat($reason->updated_at, 'date') }}</p>						
    					</div>                
    				</div>
    				@endif
                 @endif

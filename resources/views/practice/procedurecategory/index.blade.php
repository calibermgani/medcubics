...
<div class="row toolbar-header"><!-- Toolbar row Starts -->
    <section class="content-header">
        <h1>
            <small class="toolbar-heading"><i class="fa fa-medkit font14"></i> {{$heading}} <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> Account Preference <i class="fa fa-angle-double-right med-breadcrum" data-name="angle-wide-right"></i> <span>Procedure Category</span></small>
        </h1>
        <ol class="breadcrumb">
            ...
            <li class="dropdown messages-menu hide"><a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown"><i class="fa {{Config::get('cssconfigs.common.export')}}" data-placement="bottom"  data-toggle="tooltip" data-original-title="Export"></i></a>
                @include('layouts.practice_module_export', ['url' => 'api/procedurecategory/'])
            </li>
				...
				<div class="table-responsive">
					<table id="example1" class="table table-bordered table-striped ">         
						...
						<tbody>
							...
							<?php $pro_cat_id = App\Http\Helpers\Helpers::getEncodeAndDecodeOfId($pro_cat->id,'encode'); ?>
							<tr  @if($checkpermission->check_url_permission('statementholdreason/{statementholdreason}') == 1) data-url="{{ url('procedurecategory/'.$pro_cat_id) }}" @endif class="js-table-click clsCursor">

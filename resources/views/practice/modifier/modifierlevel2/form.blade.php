<input type="hidden" class="js_set_confirm_msg" value='{{ trans("practice/practicemaster/confirmmessage.validation.modifier2") }}' />
	...
	<div class="box box-info no-shadow"><!-- Box Starts -->
		...
		<div class="box-body  form-horizontal margin-l-10"><!-- Box Body Starts -->
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
				...
				@if($checkpermission->check_url_permission('modifierlevel2/delete/{id}'))
				<a class="btn btn-medcubics js-delete-confirm hide"data-text="Are you sure to delete the entry?" href="{{ url('modifierlevel2/delete/'.$modifiers->id) }}">Delete</a>
				@endif
				<a href="javascript:void(0)" data-url="{{url('modifierlevel2/'.$modifiers->id)}}">{!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
				@else
				<a href="javascript:void(0)" data-url="{{url('modifierlevel2')}}">{!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
				@endif
			</div>
		</div><!-- Box Body Ends -->
	</div><!-- /.box -->

        ...
        <ol class="breadcrumb">
            ...
            @if(count($notes) > 0)
                <li class="dropdown messages-menu hide"><a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown"><i class="fa {{Config::get('cssconfigs.common.export')}}" data-placement="bottom"  data-toggle="tooltip" data-original-title="Export"></i></a>
                    @include('layouts.practice_module_export', ['url' => 'api/facilitynodesreports/facility/'.$facility->id.'/'])
                </li>
            @endif
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin-t-20">
	<div class="box box-info no-shadow">
		<div class="box-block-header margin-b-10">
			<i class="fa fa-sticky-note med-green"></i><h3 class="box-title">Notes</h3>
			<div class="box-tools pull-right margin-t-2">
				@if($checkpermission->check_url_permission('facility/{id}/notes/create') == 1)
					<a class="js-notes font600 font14 margin-r-10" href="#"  data-toggle = 'modal' data-target="#create_notes" data-url="facility/{{$facility->id}}/notes/create" tabindex="-1"><i class="fa {{Config::get('cssconfigs.common.plus_circle')}}"></i> New Note</a>
				@endif	
			</div>
		</div><!-- /.box-header -->
		<!-- form start -->
					...
					<div class="msg-time-chat">
						...
						<div class="message-body msg-in">
							<div class="text notes box-comments">
								<p class="attribution">
									<a href="{{ url('facility/'.$facility->id.'/notes/'.$note->id.'/edit') }}">{{ $note->title }} </a>  
									<span class='notesdate'>
										@if($checkpermission->check_url_permission('notes/{notes}/edit') == 1)
									<a class="js-notes" href="#"  data-toggle = 'modal' data-target="#create_notes" data-url="facility/{{$facility->id}}/notes/{{$note->id}}/edit" tabindex="-1"><i class="livicon tooltips" style="margin-right: 2px; margin-left: 3px;" data-placement="bottom"  data-name="edit" data-color="#009595" data-size="17" data-title='Edit Note' data-hovercolor="#009595" title="Edit"></i></a>
									@endif												@if($checkpermission->check_url_permission('facility/{facility_id}/notes/delete/{id}') == 1)
										<a href="{{ url('facility/'.$facility->id.'/notes/delete/'.$note->id) }}" class="js-delete-confirm" data-text="Are you sure to delete the entry?"><i class="livicon tooltips" data-placement="bottom"  data-name="trash" data-color="#009595" data-size="16" data-title='Delete Note' data-hovercolor="#009595"></i></a>

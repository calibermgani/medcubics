    ...
    <div class="box box-info no-shadow"><!-- General Info Box Starts -->
        ...
        <div class="box-body form-horizontal margin-l-10">
            <div class="form-group">
                {!! Form::label('sub_status_desc', 'Claim Sub Status', ['class'=>'col-lg-3 col-md-3 col-sm-4 col-xs-12 control-label star']) !!}  
                <div class="col-lg-4 col-md-6 col-sm-8 col-xs-12 @if($errors->first('sub_status_desc')) error @endif">
                    <?php // text box change to textarea the reason is open pop-up but, At the same time form is submited so changed
                    ?>
                    {!! Form::text('sub_status_desc',null,['class'=>'form-control','maxlength'=>'25','autocomplete'=>'off']) !!}
                    {!! $errors->first('sub_status_desc', '<p> :message</p>') !!}
                </div>                
            </div>  
            ...
            <div class="col-lg-12 col-md-12  col-sm-12 col-xs-12 text-center">
				...
				@if(strpos($currnet_page, 'edit') !== false)
					@if($checkpermission->check_url_permission('claimsubstatus/{id}/delete') == 1)
					<a class="btn btn-medcubics js-delete-confirm"data-text="Are you sure to delete the entry?" href="{{ url('claimsubstatus/'.$claimsubstatus_encId.'/delete') }}">Delete</a>
					@endif
					<a href="javascript:void(0)" data-url="{{ url('claimsubstatus/'.$claimsubstatus_encId)}}"> {!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
				@else
					<a href="javascript:void(0)" data-url="{{ url('claimsubstatus') }}"> {!! Form::button('Cancel', ['class'=>'btn btn-medcubics js_cancel_site']) !!}</a>
				@endif
			</div>	   
        </div><!-- /.box-body -->
        ...
        $('#js-bootstrap-validator').bootstrapValidator({
            ...
            fields: {
				sub_status_desc: {
                    validators: {
                        notEmpty: {
                            message: 'Please enter claim sub status'
                        },
                        /*regexp: {
                            regexp: /^[A-Za-z \s]+$/,

...
<html>
    ...
    <body> 
		...
		<div id="compose_mail" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding hide"> 
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="background:#e7fcfd;margin-top:-10px;padding:10px 0px;">
				<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1">
					<a class="col-lg-11 col-md-11 col-sm-11 col-xs-11 med-btn" data-message-type="send" id="send_compose_message" data-page-type="forward" data-previous-id="{!! $message_data[0]->message_id !!}" style="padding:16px 10px;">
				...
				<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1">
					<a class="col-lg-9 col-md-9 col-sm-9 col-xs-9 med-btn" id="send_compose_message" data-message-type="draft" data-message-id="{!! $message_data[0]->message_id !!}" data-page-type="forward" style="padding:10px 5px;">
        ...
        <script>
			...
			<?php if($message_data[0]->message_detail->attachment_file == ''){ ?>
				var message_data = '<br><br><br><hr><div><p><label>From:</label> <span>{!! $message_data[0]->sender_details->name !!} [mailto:{!! $message_data[0]->sender_details->email !!}]</span></p><p><label>Sent:</label> <span>{!! date("D , M d,Y H:i A",strtotime($message_data[0]->created_at)) !!}</span></p><p><label>To:</label> <span>"{!! Auth::user()->name !!}"; {!! Auth::user()->email !!}</span></p><p><label>Subject:</label> <span>{!! $message_data[0]->message_detail->subject !!}</span></p><div class="message_container">{!! preg_replace("/[\r\n]*/","",$message_data[0]->message_detail->message_body); !!}</div></div>';
			<?php }else{ ?>
				var url = '{!! url("media/private_message/".$message_data[0]->message_detail->attachment_file); !!}';
				var message_data = '<br><br><br><hr><div><p><label>From:</label> <span>{!! $message_data[0]->sender_details->name !!} [mailto:{!! $message_data[0]->sender_details->email !!}]</span></p><p><label>Sent:</label> <span>{!! date("D , M d,Y H:i A",strtotime($message_data[0]->created_at)) !!}</span></p><p><label>To:</label> <span>"{!! Auth::user()->name !!}"; {!! Auth::user()->email !!}</span></p><p><label>Subject:</label> <span>{!! $message_data[0]->message_detail->subject !!}</span></p><p><a target="_blank" href="'+url+'"><label>Attachment</label> </a></span></p><div class="message_container">{!! preg_replace("/[\r\n]*/","",$message_data[0]->message_detail->message_body); !!}</div></div>';
			<?php } ?>
